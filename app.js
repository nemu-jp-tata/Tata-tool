...
function getMonsterType(data) {
  ...
  const speciesBase = allMonsters.find(m => m.species === data.species);
...
function setLevelMode(enabled) {
  ...
  if (enabled) {
    ...
    document.querySelectorAll('.monster-item.active').forEach(item => {
...
function fillCellWithMonster(cell, data) {
  ...
  const previousLevel = data.level || cell.dataset.level || null;
  cell.className = data.className || 'cell';
  ...
  // --------------------------------------
  cell.innerHTML = `
    <img class="placed-monster-image" src="${data.src}" alt="" data-src="${data.src}" data-species="${data.species || ''}" data-tier="${data.tier || ''}" data-player="${data.player || ''}" data-type="${type || ''}" draggable="false" style="max-width: 100%; max-height: 100%; width: auto; height: auto; object-fit: contain; pointer-events: auto; touch-action: none; display: block; margin: auto;" onerror="if(!this.dataset.retry){this.dataset.retry=1;this.src='${data.src}'.replace(/\\.webp$/i,'.png');}else if(this.dataset.retry=='1'){this.dataset.retry=2;this.src='${data.src}'.replace(/\\.webp$/i,'.jpg');}else{this.outerHTML='<div class=\\'no-image-badge\\' data-species=\\'${data.species || ''}\\' data-tier=\\'${data.tier || ''}\\' data-player=\\'${data.player || ''}\\'>🌸 no image 🌸</div>';}">
  ...
  targetEl.addEventListener('pointerdown', (e) => {
    ...
    const monsterType = targetEl.dataset && targetEl.dataset.type ? targetEl.dataset.type : type;
    const cellLevel = cell.dataset.level || null;
    ...
    function onUp(upEvent) {
      ...
      const existingMonster = getCellMonsterElement(targetCell);
      let targetCellData = null;
      if (existingMonster) {
        targetCellData = {
          src: existingMonster.dataset && existingMonster.dataset.src ? existingMonster.dataset.src : existingMonster.src,
          species: existingMonster.dataset ? existingMonster.dataset.species : '',
          tier: existingMonster.dataset ? existingMonster.dataset.tier : '',
          player: existingMonster.dataset ? existingMonster.dataset.player : '',
          type: existingMonster.dataset && existingMonster.dataset.type ? existingMonster.dataset.type : getMonsterType({
            species: existingMonster.dataset ? existingMonster.dataset.species : '',
            tier: existingMonster.dataset ? existingMonster.dataset.tier : ''
          }),
          level: targetCell.dataset.level || null,
      ...
      cells.forEach((c, idx) => {
        ...
        const im = getCellMonsterElement(c);
...
function revertToSourceCell() {
  if (!draggingItem || draggingItem.type !== 'board') {
    return;
  }
  const cells = Array.from(mainGrid.children);
...
function buildBoard(gridType) {
  ...
  for (let i = 0; i < totalCells; i++) {
    ...
    cell.addEventListener('click', (e) => {
      e.stopPropagation();
      if (levelMode && currentGridType === '6') {
        const monsterEl = getCellMonsterElement(cell);
        if (!monsterEl) {
          return;
        }
        let currentLevel = parseInt(cell.dataset.level || '0', 10);
        currentLevel++;
        if (currentLevel > MAX_LEVEL) {
          currentLevel = 0;
        }
        setCellLevel(cell, currentLevel);
        saveBoardState();
        return;
      }
      if (!currentSelected) {
      ...
      document.querySelectorAll('#mainGrid .cell').forEach(c => {
        ...
        if (im) {
          totalCount++;
          if (im.dataset && im.dataset.player === '1P') {
            p1Count++;
          }
          if (im.dataset && im.dataset.player === '2P') {
            p2Count++;
          }
        }
      });
      const existingImg = getCellMonsterElement(cell);
      const isReplacingSelf = existingImg && existingImg.dataset && existingImg.dataset.species === targetSpecies && (currentGridType !== '6' || existingImg.dataset.player === currentPlayer);
      if (!isReplacingSelf) {
      ...
      fillCellWithMonster(cell, {
        ...
        type: currentSelected.type || getMonsterType({
          species: targetSpecies,
          tier: targetTier
        }),
    ...
    item.addEventListener('pointerdown', (e) => {
      ...
      if (currentSelected && currentSelected.species === species) {
        paletteItemData = {
          ...
          type: currentSelected.type || baseM.type
        };
      } else {
        ...
        if (!isDragging) {
          if (Math.hypot(dx, dy) > 3) {
            ...
            draggingItem = {
              ...
              monsterType: paletteItemData.type
            };
      ...
      function onUp(upEvent) {
          ...
          if (currentSelected && currentSelected.species === species) {
            ...
            if (currentIndex < availableTiers.length - 1) {
              ...
              if (nextM) {
                ...
                currentSelected.type = nextM.type;
              }
            } else {
              currentSelected = null;
            }
        ...
        if (targetCell && mainGrid.contains(targetCell)) {
          ...
          const isReplacingSelf = existingMonster && existingMonster.dataset && existingMonster.dataset.species === targetSpecies && (currentGridType !== '6' || existingMonster.dataset.player === draggingItem.player);
          if (!isReplacingSelf) {
